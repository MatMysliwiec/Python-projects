...
class MoveMouse:
    def __init__(self, master):
        ...
        self.run_button = tk.Button(self.master, text="Start", command=self.initial)
        self.run_button.pack(padx=40, pady=5)

        self.resume_button = tk.Button(self.master, text="Resume", command=self.start)
        self.resume_button.pack(side=tk.LEFT, padx=40, pady=5)

        self.stop_button = tk.Button(self.master, text="Stop", command=self.stop)
        self.stop_button.pack(side=tk.RIGHT, padx=40, pady=5)
    ...
    def move_mouse(self):
        ...
        if self.running:
            ...
            if datetime.now() >= self.start_datetime + timedelta(hours=1):
